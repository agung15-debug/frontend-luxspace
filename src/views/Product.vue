<template>
  <div class="product">
    ...
    <section class="container mx-auto">
      <div class="flex flex-wrap my-4 md:my-12 text-left">
        <div class="w-full md:hidden px-4">
          <h2 class="text-5xl font-semibold">{{ productDetails.name }}</h2>
          <span class="text-xl">IDR {{ productDetails.price }}</span>
        </div>
        <div class="flex-1">
          <div class="slider">
            <div class="thumbnail">
              <div class="px-2">
                <div
                  ...
                  :class="item.photo == gambar ? 'selected' : ''"
                >
                  <img
                    :src="item.photo"
                    alt="front"
        ...
        <div class="flex-1 px-4 md:p-6">
          <h2 class="text-5xl font-semibold">{{ productDetails.name }}</h2>
          <p class="text-xl">IDR {{ productDetails.price }}</p>
          <router-link to="/cart" @click="moveUp">
            <a
              @click="
                saveKeranjang(
                  productDetails.id,
...
export default {
  ...
  mounted() {
    ...
    axios
      .get("http://luxspace.test/api/products", {
        params: {
